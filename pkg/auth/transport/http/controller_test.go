...
func TestController_login(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			setFingerprint:       true,
			expectedStatusCode:   http.StatusBadRequest,
			expectedResponseBody: `{"code":"CM0006","message":"validation error","data":{"password":"failed on the 'required' tag","username":"failed on the 'required' tag"}}`,
		},
		{
			...
			expectedStatusCode:   http.StatusBadRequest,
			expectedResponseBody: `{"code":"CM0006","message":"validation error","data":{"X-Fingerprint":"failed on the 'required' tag"}}`,
		},
		{
...
func TestController_logout(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			requestBody:          `{}`,
			expectedStatusCode:   http.StatusBadRequest,
			expectedResponseBody: `{"code":"CM0006","message":"validation error","data":{"refresh_token":"failed on the 'required' tag"}}`,
		},
		{
			name:        "Internal server error",
			requestBody: `{"refresh_token":"qGVFLRQw37TnSmG0LKFN"}`,
